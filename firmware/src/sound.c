...
#define UDP_MESSAGE_CURRENT_PAYLOAD_SIZE_OFFSET 4
#define UDP_MESSAGE_CURRENT_ID_OFFSET 8
#define UDP_MESSAGE_CURRENT_HOUR_OFFSET 10
#define UDP_MESSAGE_CURRENT_MINUTE_OFFSET 11
#define UDP_MESSAGE_CURRENT_SECOND_OFFSET 12
#define UDP_MESSAGE_CURRENT_MS_OFFSET 13
#define UDP_MESSAGE_CURRENT_US_OFFSET 15
...
static void initializeUdpMessageHeader()
{
    ...
    *(uint32_t*)(udpMessageData + UDP_MESSAGE_CURRENT_PAYLOAD_SIZE_OFFSET) =
...
static void updateUdpMessageIdAndTimestamp()
{
    ...
    *(uint16_t*)(udpMessageData + UDP_MESSAGE_CURRENT_ID_OFFSET) = htons(currentId);
    ...
    udpMessageData[UDP_MESSAGE_CURRENT_HOUR_OFFSET] = (uint8_t)timeinfo.tm_hour;
    udpMessageData[UDP_MESSAGE_CURRENT_MINUTE_OFFSET] = (uint8_t)timeinfo.tm_min;
    udpMessageData[UDP_MESSAGE_CURRENT_SECOND_OFFSET] = (uint8_t)timeinfo.tm_sec;
    *(uint16_t*)(udpMessageData + UDP_MESSAGE_CURRENT_MS_OFFSET) = htons(tv.tv_usec / US_IN_MS_COUNT);
    *(uint16_t*)(udpMessageData + UDP_MESSAGE_CURRENT_US_OFFSET) = htons(tv.tv_usec % US_IN_MS_COUNT);
